...
import { WeatherDisplay, WeatherInfo } from './components/WeatherDisplay'
import CitiesAPI, { Cityinfo } from './http/CitiesAPI'
import WeatherAPI from './http/WeatherAPI'

function App() {
  const [weatherInfo, setWeatherInfo] = useState<WeatherInfo | null>(null)
  const [cityQuery, setCityQuery] = useState('Moscow')
  ...
  useEffect(() => {
    async function foo() {
      ...
      setActiveCity(citiesInfo[0])
      const weather = await WeatherAPI.getWeatherFromCoords(citiesInfo[0])
  ...
  return (
    <div className='container mt-12'>
      <div className='flex items-center mb-4'>
        <p className='h1 mr-2 '>Актуальная погода</p>
        <svg
